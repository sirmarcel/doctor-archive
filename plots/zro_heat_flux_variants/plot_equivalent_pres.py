...
def get_data(thermo):
    return load(gknet / f"kappa_convergence_settings_reloaded/hfs_mic_{thermo}.nc")
...
for thermo, s in settings.items():
    ...
    fig, ax = fig_and_ax(figsize=(0.6 * textwidth, 4))

    kappas = {j: data.sel(heat_flux=j) for j in js}

    for j, kappa in kappas.items():
        plot_mean_kappa(
            ax, kappa, key=kkf, label=labels[j], alpha_err=0.1, total=True, **styles[j]
        )

    setup_ax(ax, **s)
    ax.legend(loc="lower right", handlelength=5)

    savefig(fig, f"present/equi_{thermo}.pdf")
    savefig(fig, f"present/equi_{thermo}.png")
